...
public class CassandraTraderDAO {
	...
	public CassandraTraderDAO(final String dataCenter, final int newPort,
			final String... nodes) {
		// get Cassandra connection
		client.connectCluster(newPort, "packtcdma", dataCenter, nodes);
	...
	public void saveQuote(Quote q) {
		...
		client.getSession().execute(
				bs.bind(q.getSymbol(), q.getPrice_time(), q.getOpen_price(),
						...
						q.getClose_price(), q.getVolume(), q.getSymbol_name()));
	}
	...
	public List<Quote> selectQuoteBySymbolAndDateRange(final String symbol,
		...
		for (Row r : quoteResults) {
			quotes.add(new Quote(r.getString("symbol"),
					r.getDate("price_time"), r.getFloat("open_price"), r
							.getFloat("high_price"), r.getFloat("low_price"), r
							.getFloat("close_price"), r.getDouble("volume"), r

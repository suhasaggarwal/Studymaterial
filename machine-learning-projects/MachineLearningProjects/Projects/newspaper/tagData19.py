...
text1="Flats 1 BHK Apartment at Yamuna Expressway GNoida Laxis id"
...
for x in jData:
    ...
    for x in category:
        for y in category:
            if wns.word_similarity(y, x, 'res') > 2.0:
                if x != y:
    ...
    for row1 in reader5:
        ...
        # print(entity_sim.similarity(x,y))
            if wns.word_similarity(sp[0], x, 'res') > 2.0:
    ...
    for row2 in reader4:
        ...
        for x in category:
            print(" in loop")
        # print(entity_sim.similarity(x,y))
            if wns.word_similarity(sp1[0], x, 'res') > 2.0:
    ...
    for row3 in reader3:
        ...
        sp2 = row3a.split(',')
        for x in category:
            if wns.word_similarity(sp2[0], x, 'res') > 2.0:
    ...
    for row4 in reader2:
        ...
        sp3 = row4a.split(',')
        for x in category:
            if wns.word_similarity(sp3[0], x, 'res') > 2.0:
    ...
    for row5 in reader1:
        ...
        sp4 = row5a.split(',')
        for x in category:
            if wns.word_similarity(sp4[0], x, 'res') > 2.0:

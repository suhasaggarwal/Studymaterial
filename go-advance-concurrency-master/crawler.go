...
//Context can be set with deadline to put a timer on crawling job to terminate it after a certain period
	...
	// Question --> Why number of go-routines equal to numCPUs
	// More fetch calls/queries such as ES query can be executed while go routines are waiting for response to avoid Idle time of CPUs
	numWorkers := 4
	start1 := time.Now()
...
//Fanning out the crawling jobs to multiple go routines equal to number of CPUs

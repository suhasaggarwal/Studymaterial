...
for x in rows:
    ...
    values.append(title.strip()+","+brand+","+seller+","+categorypath1+","+productImage+","+x)
    print(x)
with con:

     cur = con.cursor()

     cur.executemany("Update ProductDetails SET ProductTitle='%s',Brand='%s',Seller='%s',EcommerceSegment='%s',ProductImage='%s' WHERE ProductUrl = '%s'",values)

...
for row2a in reader4:
    ...
    for x in category:
        ...
        r = requests.get("http://101.53.130.215:8080/SemanticClassifierv2/getTextAnalysis?text=" + sp1[0])
        jData = json.loads(r.content)
        for k in jData:
            y = k['Title']
            print(y)
...
for row3a in reader3:
    ...
    for x in category:
        ...
        if wns.word_similarity(y, x, 'res') > 0.2:
           print("Word Similarity:" + y + "," + x)
           print(wns.word_similarity(y, x, 'res'))
...
for row4 in reader2:
    ...
    sp3 = row4a.split(',')
    for x in category:
       if wns.word_similarity(sp2[0], x, 'res') > 0.5:

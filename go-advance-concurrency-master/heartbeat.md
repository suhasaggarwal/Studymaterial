...
```
//--> time.After channel, if data is received on this channel,i.e go routine timed out sending a heartbeat, then it logs go routine is not healthy and can take appropriate action  
...
```
//Every goroutine is sending a heartbeat before doing a unit of work
//Every go routine is sending a heartbeat at fixed intervals for long running tasks 

...
func main() {
	...
	numWorkers := 10
	start := time.Now()
	chain := NewChain(ctx, efficientgeneratorthrottling(ctx, 10000))
	chain.
		Throttle(20, true).
...
//time taken for 5 worker 10.071967806s without throttle (5)
//time taken for 5 worker 10.06825014s with throttle (5)
//time taken for 10 worker 10.070318329s with throttle (5)
//time taken for 10 worker 10.069926215s without throttle (5)
//Sliding window possibilities
...
//Drain the channel when generation is stopped
...
//Context can be used for cancelling a generator
//Can set a deadline or can cancel it when it is required
func efficientgeneratorthrottling(ctx context.Context, limit int) chan interface{} {
	outStream := make(chan interface{})

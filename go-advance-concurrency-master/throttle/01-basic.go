...
func main() {
	...
	doWork := func(
		...
		go func() {
			for k, v := range values {
				...
				//When number of go routines ( =numCPU) have started, buffered  channel will block on receive, so more number of go routines cannot be started
				// Need not always be equal to numCPus can be a threshold also..
				worker <- k
	...
	defer close(worker)
	//Select case returns when data is signalled on the done channel, it returns immediately when it receives data from the channel, so closing the channel is suffice
	defer close(done)

	t0 := time.Now()
	//Range use call function directly which returns a channel
	...
	// fans out till maximum number of CPUs

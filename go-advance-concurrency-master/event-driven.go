...
//Question is --> what is the purpose of two select case
		go func() {
	...
	//When one of the channel does not have value (i.e condition was not met) it will get closed via copier and for range loop will exit and done will be called on the wait group, wait condition will be met finally
			for i := range c {
				select {
				case <-done:
					return
    ...
    //Question - copies the same value to two different channels
    //Applying two functions on the same value received from the stream concurrently.. to the same value
    //Mergers the two streams, supply values to multiplexed go routines which process values from two respective filters
    //How to prevent go routine leaks in this scenario, proper closing of channels

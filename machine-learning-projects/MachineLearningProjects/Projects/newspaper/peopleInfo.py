    ...
    try:
        ...
        for line in content:
            ...
            try:
                ...
                continue
            # Skip if done
            if os.path.exists("./peopleInfoboxes/"+line.strip().replace('/wiki/','')+".txt"):continue
            st = ''
                ...
                for table in tables:
                    if len(table.attrs) > 0 and table.attrs[0][1].count('infobox'):    # Find the infobox table
                        ...
                        for row in rows:
                            ...
                            if numAttrs == 1: continue
                            elif numAttrs == 2:        
                                for h in head: st += clean(str(h)) + ' | '
                                for r in row: st += clean(str(r)) + ' | '
                                st += '\n'
                                buff = []
                            # Matrix mode
                            elif numAttrs > 2 and prevNumAttrs == numAttrs:
                                tempList = []
                                for h in head: tempList.append(clean(str(h)))
                                for r in row: tempList.append(clean(str(r)))
                        ...
                        if prevNumAttrs > 2:
                            ...
                            for i in range(1,len(buff)):
                                for j in range(1,len(buff[0])):
                                    st += buff[0][j]+' - '+buff[i][0] + ' | ' + buff[i][j]
                                    st += '\n'
            except Exception as e:
                print e  
            # Print results              
            try:
                f = open ("./peopleInfoboxes/"+line.strip().replace('/wiki/','')+".txt",'w')
                f.write(st)
                f.close()
            except:
